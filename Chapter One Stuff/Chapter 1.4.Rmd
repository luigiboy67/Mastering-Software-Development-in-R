...
Simplest way to read web-based data is if the data is available online as a flat file.
...
You can use some `dplyr` functions to check out the data set once it's in R like this:
...
you can also read in flat lines from secure urls (https).
...
For R users, APIs provide rules you need to know to write R code to request and pull data from the organization's web server into your R session.
...
Many organizations will require you to get an API key and use this key in each of your API requests.
...
When using `GET` or `POST` functions from the `httr` package, you include the key-value pairs for any query parameters as a list object in the `query` argument of the function.
...
There are other packages for API which includes:
...
You can also use R to pull and cran web-based data that is not accessible through a web API or as an online flat file.

The `rvest` package is a good entry point for handling more complex collection and cleaning of web-based data.

Look up the [rvest Github README](https://github.com/tidyverse/rvest).
