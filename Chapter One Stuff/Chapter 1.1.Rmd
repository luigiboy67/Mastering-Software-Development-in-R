...
So entering `1` in r gives you a numeric object; entering `1L` explicitly gives you an integer object.
...
Often factors will be automatically created for you when you read a data set in using a function like `read.table()`.
...
Column names and row names can be set separately using `colnames()` and `rownames()` functions.
...
Attributes of an object (if any) can be accessed using the `attributes()` function. Not all R objects contain `attributes()`, which can return `NULL`
